...
test('mobile progressive disclosure does not overlap controls',async({page})=>{
  ...
  await expect(page.locator('footer')).toBeHidden();await expect(page.locator('#assetMode')).toBeVisible();await expect(page.locator('#frameMode')).toBeVisible();await expect(page.locator('#moreToggle')).toBeVisible();await expect(page.locator('#inspectorToggle')).toBeVisible();
  await page.locator('#frameMode').click();await expect(page.locator('footer')).toBeVisible();
...
test('iPhone Frame mode keeps navigation, tools, and timeline reachable',async({page})=>{
  ...
  for(let cycle=0;cycle<5;cycle++){
    ...
    expect(visibleTools).toEqual(['move','rotate','erase','fill','inspectorToggle']);
    ...
    await page.locator('#assetMode').click();await expect(page.locator('footer')).toBeHidden();await expect(page.locator('#assetMode')).toHaveClass(/active/);await expect(page.locator('#frameMode')).toBeVisible();await expect(page.locator('#moreToggle')).toBeVisible();
  }
  expect(errors).toEqual([]);
});
...
test('Head, frame-local Fill layers, and Demo walk remain contextual and persistent',async({page})=>{
  ...
  await page.locator('#inspectorToggle').click();const fillLayer=page.locator('[data-fill]').filter({hasText:'Fill 1'});await expect(fillLayer).toHaveCount(1);await expect(page.locator('#assets .asset').last()).toContainText('Fill 1');await fillLayer.click();await expect(page.locator('#fillWidth')).toBeVisible();expect(await page.evaluate(()=>window.SpriteStudioTest.activeTarget().type)).toBe('fill');await page.locator('#inspectorClose').click();
