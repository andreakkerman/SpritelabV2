...
test('Head, joint repair, layers, and Demo walk remain contextual and persistent',async({page})=>{
  ...
  await expect(page.locator('#addRepair')).toBeVisible();await expect(page.locator('#pivotX')).toBeHidden();await expect(page.locator('#zIndex')).toBeHidden();await expect(page.locator('#brushSize')).toBeHidden();await expect(page.locator('.advanced-only').filter({hasText:'Seam QA'})).toBeHidden();await page.locator('#advancedInspector').click();await expect(page.locator('#pivotX')).toBeVisible();await page.locator('#basicInspector').click();await expect(page.locator('#pivotX')).toBeHidden();
  await page.locator('#addRepair').click();await expect(page.locator('[data-repair]')).toContainText('Repair 1');await expect(page.locator('#repairSize')).toHaveValue('medium');await expect(page.locator('#repairShape')).toHaveValue('round');await page.locator('#setRepairPoint').click();const canvas=page.locator('#canvas'),box=await canvas.boundingBox();await canvas.click({position:{x:box.width/2,y:box.height/2}});
  expect(await page.evaluate(()=>{const p=window.SpriteStudioTest.project(),selectedId=window.SpriteStudioTest.selectedAssetId(),repair=p.repairs?.left?.[0];return{repair,hasUpper:p.rigAssets.some(a=>a.role==='left_upper_leg'),hasLower:p.rigAssets.some(a=>a.role==='left_lower_leg'),selectedRole:p.rigAssets.find(a=>a.id===selectedId)?.role}})).toMatchObject({repair:{assetId:expect.any(String),point:{x:expect.any(Number),y:expect.any(Number)}},hasUpper:true,hasLower:true,selectedRole:'left_upper_leg'});
  await page.locator('#inspectorToggle').click();await page.locator('[data-repair]').filter({hasText:'Repair 1'}).click();await page.locator('#repairColor').fill('#ff0000');await page.locator('#repairSize').selectOption('xl');await page.locator('#repairShape').selectOption('capsule');await page.locator('#autoRepairColor').click();await page.locator('.asset[data-id]').filter({hasText:'Left upper leg'}).click();await page.locator('#addRepair').click();await expect(page.locator('[data-repair]')).toHaveCount(2);await page.locator('.asset[data-id]').filter({hasText:'Left upper leg'}).click();await expect(page.locator('#addRepair')).toBeDisabled();expect(await page.evaluate(()=>{const repair=window.SpriteStudioTest.project().repairs.left[0];return{size:repair.size,shape:repair.shape,colorMode:repair.colorMode}})).toEqual({size:'xl',shape:'capsule',colorMode:'automatic'});await page.locator('[data-repair]').filter({hasText:'Repair 2'}).click();await page.locator('#deleteRepair').click();await expect(page.locator('[data-repair]')).toHaveCount(1);
  await page.locator('[data-id]').filter({hasText:'Head'}).click();await page.locator('#bringForward').click();await page.locator('#sendBackward').click();await page.locator('#inspectorClose').click();const projectBeforeBackground=await page.evaluate(()=>JSON.stringify(window.SpriteStudioTest.project()));await page.locator('#canvasBackground').selectOption('black');await expect(page.locator('#viewport')).toHaveAttribute('data-background','black');await page.locator('#canvasBackground').selectOption('white');await expect(page.locator('#viewport')).toHaveAttribute('data-background','white');expect(await page.evaluate(()=>JSON.stringify(window.SpriteStudioTest.project()))).toBe(projectBeforeBackground);await expect(page.locator('#timelineSettings')).toBeHidden();await page.locator('#moreToggle').click();await expect(page.locator('#morePanel [data-demo-walk]')).toBeVisible();await page.locator('#morePanel [data-demo-walk]').click();await expect(page.locator('#demoDialog')).toBeVisible();await expect(page.locator('#demoDirection')).toHaveValue('right');await page.locator('#demoDirection').selectOption('ping-pong');await page.locator('#demoSpeed').selectOption('fast');const geometry=await page.evaluate(()=>{const rect=id=>{const r=document.querySelector(id).getBoundingClientRect();return{left:r.left,top:r.top,right:r.right,bottom:r.bottom}};return{dialog:rect('#demoDialog'),canvas:rect('#demoCanvas'),close:rect('#closeDemo'),controls:['#demoPlay','#demoDirection','#demoSpeed','#closeDemo'].map(rect)}});expect(geometry.canvas.bottom).toBeLessThanOrEqual(geometry.close.top);for(const control of geometry.controls){expect(control.left).toBeGreaterThanOrEqual(geometry.dialog.left);expect(control.right).toBeLessThanOrEqual(geometry.dialog.right);expect(control.top).toBeGreaterThanOrEqual(geometry.dialog.top);expect(control.bottom).toBeLessThanOrEqual(geometry.dialog.bottom)}await page.locator('#demoPlay').click();await expect(page.locator('#demoPlay')).toHaveText('Play');await page.locator('#demoPlay').click();await expect(page.locator('#demoPlay')).toHaveText('Pause');const before=await page.evaluate(()=>JSON.stringify(window.SpriteStudioTest.project()));await page.waitForTimeout(150);await page.locator('#closeDemo').click();expect(await page.evaluate(()=>JSON.stringify(window.SpriteStudioTest.project()))).toBe(before);
