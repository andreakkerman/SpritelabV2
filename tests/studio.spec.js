...
test('Studio sheets share dismissal and exclusivity behavior',async({page})=>{
  ...
  await page.locator('#moreToggle').click();await expect(more).toHaveClass(/open/);await expect(page.locator('#sheetBackdrop')).toBeVisible();
  await page.locator('#moreToggle').click();await expect(more).not.toHaveClass(/open/);
  await page.locator('#moreToggle').click();await page.locator('#sheetBackdrop').click({position:{x:2,y:2}});await expect(more).not.toHaveClass(/open/);
  await page.locator('#moreToggle').click();await page.locator('#inspectorToggle').click();await expect(more).not.toHaveClass(/open/);await expect(assets).toHaveClass(/open/);
  await page.locator('#inspectorClose').click();await expect(assets).not.toHaveClass(/open/);
  await page.locator('#moreToggle').click();await page.keyboard.press('Escape');await expect(more).not.toHaveClass(/open/);
});
