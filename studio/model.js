...
export function deleteRepair(project,side,id){const items=repairsFor(project,side),index=items.findIndex(item=>item.id===id);if(index<0)return false;items.splice(index,1);return true}
export function repairWorldPosition(assets,frame,repair){const owner=assets.find(a=>a.id===repair?.assetId);return owner?transformPoint(assetWorldTransform(assets,frame,owner.id),{x:owner.position.x+repair.point.x+repair.offset.x,y:owner.position.y+repair.point.y+repair.offset.y}):null}
export function coverRenderOrder(project,frame){const repairs=["left","right"].flatMap(side=>(project.repairs?.[side]||[]).map(item=>`repair:${side}:${item.id}`));const legacy=["left","right"].filter(side=>project.jointCovers?.[side]).map(side=>`joint-cover:${side}`);const assets=[...project.rigAssets].sort((a,b)=>(frame?.assetOverrides?.[a.id]?.zIndex??a.zIndex)-(frame?.assetOverrides?.[b.id]?.zIndex??b.zIndex)).map(a=>a.id);return[...repairs,...legacy,...assets]}
...
export function createProject(name="Untitled Sprite"){return{schemaVersion:2,project:{id:crypto.randomUUID(),name,canvas:{width:360,height:440}},master:null,rigAssets:[],frames:Array.from({length:8},(_,index)=>blankFrame(index)),fps:10,activeFrame:0,joints:{},jointCovers:{},repairs:{}}}
