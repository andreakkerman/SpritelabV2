...
export const JOINT_SIZES={small:10,medium:16,large:22};
...
export function coverRenderOrder(project,frame){const sorted=[...project.rigAssets].sort((a,b)=>(frame?.assetOverrides?.[a.id]?.zIndex??a.zIndex)-(frame?.assetOverrides?.[b.id]?.zIndex??b.zIndex));const out=[];for(const asset of sorted){for(const side of ["left","right"]){const cover=project.jointCovers?.[side],chain=resolveLegChain(project.rigAssets,`${side}_upper_leg`);if(cover&&chain&&asset.id===(sorted.filter(a=>a.id===chain.upper?.id||a.id===chain.lower?.id)[0]?.id))out.push(`joint-cover:${side}`)}out.push(asset.id)}return out}
...
export function createProject(name="Untitled Sprite"){return{schemaVersion:2,project:{id:crypto.randomUUID(),name,canvas:{width:360,height:440}},master:null,rigAssets:[],frames:Array.from({length:8},(_,index)=>blankFrame(index)),fps:10,activeFrame:0,joints:{},jointCovers:{}}}
