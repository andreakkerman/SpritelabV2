...
export function blankFrame(index=0){return{id:crypto.randomUUID(),name:`Frame ${index+1}`,durationMs:100,assetOverrides:{},repairOverrides:{},assetEraseMasks:{},fills:[],patches:[],fullImage:null}}
...
export function normalizeProject(project){project.frames||=[];for(const frame of project.frames){frame.assetOverrides||={};frame.repairOverrides||={};frame.assetEraseMasks||={};frame.fills||=[];frame.patches||=[];frame.fullImage??=null;for(const fill of frame.fills){fill.shape="circle";fill.opacity??=1;fill.visible??=true;fill.eraseMask||=[]}}/* Legacy repair data is intentionally ignored. */return project}
